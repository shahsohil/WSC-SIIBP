function [learnedModel,p_acc,a_acc] = wssiibp(para,model,xData)
...
fprintf('Iter\t Person Acc\t Action Acc\n');
...
%         Reading weak labels of current video
        ...
        for k = 1:K
            ...
            cur_nu(:,k) = (ws(k)>0)./(1+exp(-(...
                ...
                + 1/sigma_n1^2*phi(k,1:D1)*(cur_X(:,1:D1)-cur_nu*phi(:,1:D1)+cur_nu(:,k)*phi(k,1:D1))'...
                + 1/sigma_n2^2*phi(k,D1+1:end)*(cur_X(:,D1+1:end)-cur_nu*phi(:,D1+1:end)+cur_nu(:,k)*phi(k,D1+1:end))')));
...
%   Track accuracy every iteration    
    if(I == para.MAX_ITERATIONS)
        [p_acc(I),a_acc(I)] = accuracy(c_nu,1:K,xData,1);
    else
        [p_acc(I),a_acc(I)] = accuracy(c_nu,rev_ordering,xData,0);
    end   
    fprintf('%d\t%1.4f\t%1.4f\n',I,p_acc(I),a_acc(I));
end
